...
func run(args []string) (n int, err error) {
	...
	for _, name := range names {
		if !strings.HasSuffix(name, ".mixml") {
			continue
		}

		var buf bytes.Buffer
		var output bytes.Buffer
		cmd := exec.Command("./mixml-mlkit", append(args, filepath.Join(dir, name))...)
		cmd.Stderr = &buf
		cmd.Stdout = &output
		if err := cmd.Run(); err != nil {
			return n, &Error{err: err, filename: name, stderr: buf.String()}
		}
		golden := filepath.Join(dir, strings.ReplaceAll(name, ".mixml", ".golden"))
		if *update {
